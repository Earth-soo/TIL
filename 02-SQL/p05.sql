...
/* 5. VIP 고객 구매 내역
고객 유형이 'VIP' 인 고객들의 구매 상품, 주문 금액, 주문일을 조회하고
주문금액이 큰 순서로 정렬하시오. */
SELECT 
	c.customer_name AS 고객명,
	s.product_name AS 구매상품,
	s.total_amount AS 주문금액,
	s.order_date AS 주문일
FROM sales s
INNER JOIN customers c ON c.customer_id = s.customer_id
WHERE c.customer_type = 'VIP'
GROUP BY c.customer_name, s.product_name, s.order_date
ORDER BY 주문금액;
...
SELECT 
	...
	COUNT(s.order_date) AS 주문횟수,
	SUM(s.total_amount) AS 총구매금액,
	ROUND(AVG(s.total_amount), 2) AS 평균구매금액,
	MAX(s.order_date) AS 최근주문일
FROM sales s
INNER JOIN customers c ON c.customer_id = s.customer_id
GROUP BY c.customer_id, c.customer_name, c.customer_type, s.order_date
ORDER BY 평균구매금액 DESC;

/* 8. 모든 고객 구매 통계 (주문 없는 고객 포함)
모든 고객에 대해 주문횟수, 총구매금액, 평균구매금액, 최대구매금액을 계산하시오.
구매가 없는 경우 0으로 처리하시오. */
SELECT 
	c.customer_name AS 고객명,
	COALESCE(COUNT(s.order_date), 0) AS 주문횟수,
 	COALESCE(SUM(total_amount), 0) AS 총구매금액,
	COALESCE(AVG(total_amount), 0.0) AS 평균구매금액,
	COALESCE(MAX(total_amount), 0) AS 최대구매금액
FROM sales s
