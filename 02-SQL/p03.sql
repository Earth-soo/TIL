...
SELECT * FROM userinfo WHERE name LIKE '%수%' AND phone LIKE '010%' AND email LIKE '%@gmail.com';

-- 성이 김/이 둘중 하난데 gmail 씀 
-- name LIKE '김%' OR name LIKE '이%' AND email LIKE '@gmail.com' 하면 X
-- AND 우선순위가 높아서 '이'씨면서 gmail을 사용하는 사람으로 조건을 봄
SELECT * FROM userinfo WHERE (name LIKE '김%' OR name LIKE '이%') AND email LIKE '%@gmail.com';